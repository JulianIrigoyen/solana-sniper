...
async fn get_holders(request: web::Json<HolderRequest>) -> impl Responder {
...
async fn process_mint_addresses(mint_addresses: Vec<String>) -> Result<HashMap<String, usize>, Box<dyn Error>> {
    ...
    for mint_address in mint_addresses {
        ...
        let response = client
            .post("https://necessary-light-mound.solana-mainnet.quiknode.pro/<3 love you but this call is expensive >/")
            .headers(headers)
            .json(&rpc_request)
            .send()
            .await?;
